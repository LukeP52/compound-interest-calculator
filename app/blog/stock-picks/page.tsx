...
export const metadata: Metadata = {
  title: "Stock Picks & Analysis | Compound Interest Calculator",
...
export default async function StockPicksPage() {
  const category = getCategoryBySlug("stock-picks");
  const posts = await getAllPosts("stock-picks");
      ...
      <div className="relative z-10 p-4 md:p-8 lg:p-24">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-12">
            {category?.description || "Stock recommendations and analysis"}
          </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <article key={post.slug} className="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200">
                <Link href={`/blog/stock-picks/${post.slug}`} className="block h-full p-6">
          ...
          {posts.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                No stock pick posts yet. Check back soon!
              </p>
            </div>
          )}
        </div>
