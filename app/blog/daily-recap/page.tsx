...
export const metadata: Metadata = {
  title: "Daily Market Recap | Compound Interest Calculator",
...
export default async function DailyRecapPage() {
  const category = getCategoryBySlug("daily-recap");
  const posts = await getAllPosts("daily-recap");
      ...
      <div className="relative z-10 p-4 md:p-8 lg:p-24">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-4xl font-bold text-gray-800 dark:text-white mb-8">
            {category?.name || "Daily Recap"}
          </h1>
          
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-12">
            {category?.description || "Daily market recaps and analysis"}
          </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <article key={post.slug} className="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200">
                <Link href={`/blog/daily-recap/${post.slug}`} className="block h-full p-6">
          ...
          {posts.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                No daily recap posts yet. Check back soon!
              </p>
            </div>
          )}
        </div>
